...
### Tier 1 — Metadata
- **Call:** `gh repo list --json name,description,primaryLanguage,languages,stargazerCount,updatedAt,visibility,owner,isFork,repositoryTopics --limit 1000`
- **Used for:** locating repos by name/language/topic/recency; almost every question starts here.
- **Cost:** one API call regardless of question complexity.

### Tier 2 — READMEs
- **Call (per shortlisted repo):** `gh api repos/{owner}/{name}/readme -H "Accept: application/vnd.github.raw"`
...
- **Calls (targeted, per shortlisted repo):**
...
- No custom scripts, daemons, or external state — skill is purely procedural over `gh` CLI.
- No persistent cache across sessions — Tier 1 cache is conversation-scoped only.
